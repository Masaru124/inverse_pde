...
checkpoint = torch.load(
    'outputs_nonsmooth_v2_phase1_fastcheck/checkpoints/epoch_003_valnll_0.461289.pt',
    map_location=device,
    weights_only=True
)
...
print(f'TRAINED mu mean={mu.mean().item():.6f} mu std={mu.std().item():.6f}')
print(f'TRAINED mu min={mu.min().item():.6f} mu max={mu.max().item():.6f}')
...
print(f'\nRMSE estimate: {torch.sqrt(((mu - batch["k_grid"])**2).mean()).item():.6f}')
